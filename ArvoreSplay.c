...
{
    // verifica se a raiz é nula ou se o valor já está no topo.
    if (raiz == NULL || raiz->item.valor == valor)
        return raiz;

    if (raiz->item.valor > valor) // verifica se o valor vai estar na esquerda da raiz, ou se o valor procurado é menor do que a raiz
    {
        // verifica se a subarvore esquerda está vazia, caso esteja ela irá retornar.
        if (raiz->esquerda == NULL)
            return raiz;

        if (raiz->esquerda->item.valor > valor) // se o valor da subarvore esquerda for maior que o valor buscado, será feito o Zig-Zig (esquerda esquerda)
        {
            raiz->esquerda->esquerda = splay(raiz->esquerda->esquerda, valor); // a subarvore - esquerda está rotacionando usando o splay para buscar o valor usando a recursão.
            raiz = rotacaoDireita(raiz);                                       // atribuindo a raiz a nova arvore rotacionando a para a direita
        }
        else if (raiz->esquerda->item.valor < valor) // se o valor da subarvore esquerda for maior que o valor buscado, será feito o Zig-zag (direita esquerda)
        {

            raiz->esquerda->direita = splay(raiz->esquerda->direita, valor); // a subarvore - esquerda direita está rotacionando usando o splay para buscar o valor usando a recursão.

            if (raiz->esquerda->direita != NULL)                  // verificando se a subarvore é null
                raiz->esquerda = rotacaoEsquerda(raiz->esquerda); // se for verdade o if , será rotacionado a raiz esquerda para a esquerda
        }

        // Condicional utilizando o operador ternario (é tipo um If e Else)
        return (raiz->esquerda == NULL) ? raiz : rotacaoDireita(raiz);
    }
    else if (raiz->item.valor < valor) // verifica se o valor vai estar na direita da raiz, ou se o valor procurado é maior do que a raiz
    {
        // verifica se a subarvore direita está vazia, caso esteja ela irá retornar.
        if (raiz->direita == NULL)
            return raiz;

        if (raiz->direita->item.valor > valor) // se o valor da subarvore direita for maior que o valor buscado, será feito o zag-zag (direita direita)
        {
            raiz->direita->esquerda = splay(raiz->direita->esquerda, valor); // a subarvore - direita está rotacionando usando o splay para buscar o valor usando a recursão.

            if (raiz->direita->esquerda != NULL)               // verificando se a subarvore é null
                raiz->direita = rotacaoDireita(raiz->direita); // se for null a raiz direita vai rotacionar a direita
        }
        else if (raiz->direita->item.valor < valor) // se o valor da subarvore direita for menor que o valor buscado, será feito o zig-zag (esquerda direita)

        {
            raiz->direita->direita = splay(raiz->direita->direita, valor); // a subarvore - direita direita está rotacionando usando o splay para buscar o valor usando a recursão.
            raiz = rotacaoEsquerda(raiz);                                  // se for verdade o if , será rotacionado a raiz para a esquerda
        }
